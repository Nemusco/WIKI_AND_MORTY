...
document.addEventListener(
	"DOMContentLoaded",() => {
		document.querySelector("#search").addEventListener("input",searchEpisodes,false);
		document.querySelector("#search").addEventListener("blur",searchEpisodes,false);
		document.querySelector("#search").addEventListener("change",searchEpisodes,false);
...
function  getEpisodes(){
	...
	ajax.request("POST","https://wiki-and-morty.herokuapp.com/episodes","text");
}

//Esta es la funcion que muestra los resultados
function showEpisodes( results, resultados ){
	...
	results.forEach( item => {
		const { name, air_date, episode } = item;
			...
			content.innerHTML = `
			<h1 class="h3">${name}</h1>
			Air date: <span class="text-success">${air_date}</span><br><br>
			Episode code: <span class="text-info">${episode}</span><br>
			`;
...
function getPage( elem ){
	...
	ajax.request("POST","https://wiki-and-morty.herokuapp.com/episodes","text",data);
}
...
function serverRequest( status, message, response ){
	...
	showEpisodes(results,resultados);
