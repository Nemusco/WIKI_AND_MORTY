...
document.addEventListener(
	"DOMContentLoaded",() => {
		document.querySelector("#search").addEventListener("input",searchCharacters,false);
		document.querySelector("#search").addEventListener("blur",searchCharacters,false);
		document.querySelector("#search").addEventListener("change",searchCharacters,false);
...
function  getCharacters(){
	...
	ajax.request("POST","https://wiki-and-morty.herokuapp.com/characters","text");
}

//Esta es la funcion que muestra los resultados
function showCharacters( results, resultados ){
	
	cleanHTML(resultados);

	results.forEach( item => {
		const { episode, gender, id, image, location, name, origin, species, status, type, url } = item;
			...
			imageCol.classList.add("col-12","col-md-3");

			const imgCharacter = document.createElement("img");
			imgCharacter.classList.add("img-fluid","p-2");
			...
			<span class="text-right text-warning">${species}</span>
			<span class="text-right text-white">${type}</span>
			<span class="text-right text-danger">${gender}</span><br><br>
			Origin: <span class="text-info">${origin.name}</span><br>
			Location: <span class="text-info">${location.name}</span>
			`;

			imageCol.appendChild(imgCharacter);
...
function getPage( elem ){
	...
	ajax.request("POST","https://wiki-and-morty.herokuapp.com/characters","text",data);
}
...
function serverRequest( status, message, response ){
	...
	showCharacters(results,resultados);
...
function searchCharacters( elem ){
